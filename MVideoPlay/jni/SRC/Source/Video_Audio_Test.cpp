		...
		int out_sample_rate, uint8_t* out_buf)
{
	...
	if (out_channels == 1)
	{
		...
		//LOGE("dst_ch_layout: AV_CH_LAYOUT_MONO\n");
	}
	else if (out_channels == 2)
	{
		dst_ch_layout = AV_CH_LAYOUT_STEREO;
		//LOGE("dst_ch_layout: AV_CH_LAYOUT_STEREO\n");
	}
	else
	{
		dst_ch_layout = AV_CH_LAYOUT_SURROUND;
		//LOGE("dst_ch_layout: AV_CH_LAYOUT_SURROUND\n");
	}
...
/*
 * 声音回调函数  ： userdata是输入，stream是输出，len是输入，len的值一般为4096（调试中发现的），
 *  audio_callback函数的功能是调用audio_decode_frame函数，
 *  把解码后数据块audio_buf追加在stream的后面，通过SDL库对audio_callback的不断调用，不断解码数据，
 *  然后放到stream的末尾，  SDL库认为stream中数据够播放一帧音频了，就播放它,第三个参数len是向stream中写数据的内存分配尺度，
...
void decode_audio()
{
	...
	if (ren == NULL)
	{
		LOGI("SDL CreateRenderer error: %s\n", SDL_GetError());
		return;
	}
	...
	while (av_read_frame(pFormatCtx, &packet) >= 0)
	{
		...
		else if (packet.stream_index == audioIndex)
		{
			...
			/*ret = avcodec_decode_audio4(paCodecCtx, pFrame, &frameFinished,
					&packet);
			if (ret < 0)
			{
