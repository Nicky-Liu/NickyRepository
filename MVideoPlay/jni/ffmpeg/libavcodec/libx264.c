...
#include <stdlib.h>
#include <string.h>
//#define  PRINT_LIBX264   1
#if PRINT_LIBX264//  #ifdef _KVDBG_
#include <stdio.h>
#include <stdlib.h>
//#include <time.h>
...
#if PRINT_LIBX264  // #ifdef _KVDBG_
                                        if (fp == NULL)
                                          fp = fopen("/sdcard/log.txt", "w+");
                                        fprintf(fp, "enter X264_frame \n");
#endif
...
static av_cold int X264_init(AVCodecContext *avctx)
{
    ...
    x264_param_default(&x4->params);
                        ...
                        if (fp == NULL)
                          fp = fopen("/sdcard/log.txt", "w+");
                        fprintf(fp, "after x264_param_default \n");
#endif
    ...
    if (x4->preset || x4->tune)
        if (x264_param_default_preset(&x4->params, x4->preset, x4->tune) < 0) {
            ...
            return AVERROR(EINVAL);
                                ...
                                if (fp == NULL)
                                  fp = fopen("/sdcard/log.txt", "w+");
                                fprintf(fp, "after x264_param_default_preset \n");
#endif
